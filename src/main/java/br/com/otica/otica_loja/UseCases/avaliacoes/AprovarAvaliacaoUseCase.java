...
@Service
public class AprovarAvaliacaoUseCase {

    @Autowired
    private ProdutoAvaliacaoRepository avaliacaoRepository;

    /**
     * Aprova uma avaliação de produto.
     */
    public ProdutoAvaliacao aprovar(UUID avaliacaoId) {
        ProdutoAvaliacao avaliacao = avaliacaoRepository.findById(avaliacaoId)
                .orElseThrow(() -> new IllegalArgumentException("Avaliação não encontrada."));

        if (avaliacao.getAprovado()) {
            throw new IllegalArgumentException("Esta avaliação já está aprovada.");
        }
    ...
    /**
     * Reprova uma avaliação de produto (opcional).
     */
