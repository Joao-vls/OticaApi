...
public class AtualizarBannerEditorialUseCase {
    ...
    public BannerEditorial executar(
        ...
        BannerEditorial banner = bannerEditorialRepository
                ...
                .orElseThrow(() -> new IllegalArgumentException("Banner não encontrado com o identificador: " + identificador));

        banner.setLayoutTipo(layoutTipo);
        banner.setTextoMarca(textoMarca);
        banner.setAtualizadoEm(OffsetDateTime.now());

        // Atualização preservando a mídia anterior se nada novo for fornecido
        ...
        if (sec1Preco != null) banner.setSec1Preco(sec1Preco);
        if (sec1Desconto != null) banner.setSec1Desconto(sec1Desconto);
        ...
        if (sec2Preco != null) banner.setSec2Preco(sec2Preco);
        if (sec2Desconto != null) banner.setSec2Desconto(sec2Desconto);
